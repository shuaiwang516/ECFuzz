...
public class AlluxioProperties {
  ...
  private Hash mHash = new Hash(() -> keySet().stream()
      .filter(key -> getSilently(key) != null)
      .sorted(Comparator.comparing(PropertyKey::getName))
      .map(key -> String.format("%s:%s:%s", key.getName(), getSilently(key), getSource(key)).getBytes()));
  ...
  @Nullable
  private String getSilently(PropertyKey key) {
  ...
  public void putSilently(PropertyKey key, String value, Source source) {
    if (!mUserProps.containsKey(key) || source.compareTo(getSource(key)) >= 0) {
      mUserProps.put(key, Optional.ofNullable(value));
      mSources.put(key, source);
      mHash.markOutdated();
  ...
  public void merge(Map<?, ?> properties, Source source) {
    ...
    for (Map.Entry<?, ?> entry : properties.entrySet()) {
      ...
      putSilently(propertyKey, value, source);
    }
    mHash.markOutdated();
  }
  ...
  public Set<Map.Entry<PropertyKey, String>> entrySet() {
    return keySet().stream()
        .map(key -> Maps.immutableEntry(key, getSilently(key)))
        .collect(toSet());
  }
